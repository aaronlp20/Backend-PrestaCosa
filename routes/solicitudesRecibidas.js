...
// GET /api/solicitudes/recibidas/:propietarioId
router.get("/recibidas/:propietarioId", async (req, res) => {
  try {
    const solicitudes = await Solicitud.findAll({
      where: {
        propietarioId: req.params.propietarioId,
      },
      include: incluirTodo,
      order: [["id", "ASC"]],
    });
...
router.put("/:id/aprobar", async (req, res) => {
  ...
  const conflicto = await Solicitud.findOne({
    where: {
      ...
      fechaInicio: {
        [Op.lte]: solicitud.fechaFin,
      },

      fechaFin: {
        [Op.gte]: solicitud.fechaInicio,
      },
    },
  });
  ...
  if (conflicto) {
    return res.status(400).json({
      error: "Ya existe un préstamo aprobado en esas fechas",
    });
  }
